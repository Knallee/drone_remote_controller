...
uint8_t mpu6050_fifoBuffer[20] = {0};
	
volatile long	  test[4];
...
void mpu6050_getQuaternion(const uint8_t* packet, double *qw, double *qx, double *qy, double *qz) {
	test[0] = (packet[0]  << 8) | packet[1];
	test[1] = (packet[4]  << 8) | packet[5];
	test[2] = (packet[8]  << 8) | packet[9];
	test[3] = (packet[12] << 8) | packet[13];
    *qw = (double)(test[0]) / 16384.0f;
    *qx = (double)(test[1]) / 16384.0f;
    *qy = (double)(test[2]) / 16384.0f;
    *qz = (double)(test[3]) / 16384.0f;
}
...
void mpu6050_getRollPitchYaw(double qw, double qx, double qy, double qz, double *roll, double *pitch, double *yaw) {
	//*yaw = atan2(2*qx*qy + 2*qw*qz,- 2*qw*qw - 2*qx*qx + 1)*57.3;
	*yaw	= 0;
	*pitch	= -asin(2*qw*qy - 2*qx*qz)*57.3;
	*roll	= atan2(2*qw*qx + 2*qy*qz, -2*qy*qy - 2*qx*qx +1)*57.3;
}
...
//
	//if (!(!mpu6050_mpuInterrupt && mpu6050_fifoCount < MPU6050_DMP_dmpPacketSize))
	//{
		////reset interrupt
...
/*
 * on interrupt set data available
 */
ISR (INT4_vect) {
	if (MPUFlag == 2)
	{
		MPUFlag  = 1;
		readFlag = 1;
		TWI_Start_Transceiver_With_Data(MPU6050_RA_FIFO_R_W, mpu6050_fifoBuffer, 16);
	}		
}
